...
export default function CardCalendar({ color }) {
  ...
  const openReservationModal = () => {setCreateReservationIsOpen(true)};
  const closeReservationModal = () => {setCreateReservationIsOpen(false)};

  const openBlockadeModal = () => {setCreateBlockadeIsOpen(true)};
  const closeBlockadeModal = () => {setCreateBlockadeIsOpen(false)};
  ...
  const searchReservations = async() => {
    if(category === 'option' || category === '' || filter === 'option' || filter === ''){
      ...
      const blockadesResponse = await blockadesClient.getSemesterBlockades(localStorage.getItem('currentSemester-Year'), localStorage.getItem('currentSemester-Semester'));
